"""Repositorio para datos WRF-SMN desde AWS S3.

Este repositorio implementa acceso directo a AWS S3 Open Data
para obtener datos WRF-SMN del Servicio Meteorológico Nacional de Argentina.
"""
...
class WRFSMNRepository(IWeatherRepository):
    ...
    Estrategia:
    1. Acceso directo a AWS S3 (Open Data) - único método
    2. Cache local con TTL 6 horas
    3. No requiere credenciales AWS (acceso anónimo)
    """
    ...
    def __init__(self, use_meteosource_fallback: bool = False, cache_ttl_hours: int = 6):
        """
        Inicializar repositorio WRF-SMN desde AWS S3.

        Args:
            use_meteosource_fallback: Siempre False - solo usar AWS S3 (ignorado)
            cache_ttl_hours: TTL del cache local en horas (default: 6)
        """
        # Siempre False - solo usar AWS S3, sin fallback
        ...
        # Meteosource eliminado - solo usar AWS S3
    ...
    def _get_from_s3(
        ...
        try:
            ...
            # El bucket de Open Data usa acceso público
            try:
