...
def verify_model_against_observation(
    model_data: WeatherData, observation_data: WeatherData, variable: str
) -> Dict[str, Optional[float]]:
    """
    ...
    if model_value is None or obs_value is None:
        logger.warning(
            f"Valores faltantes para {variable}: model={model_value}, obs={obs_value}"
        )
...
def verify_multiple_variables(
    model_data: WeatherData, observation_data: WeatherData, variables: list
    ...
    for variable in variables:
        try:
            metrics = verify_model_against_observation(
                model_data, observation_data, variable
            )
