...
class UnifiedWeatherEngine:
    ...
    def __init__(
        ...
        self.fusion_processor = WeatherFusionProcessor(
            weights=fusion_weights, inconsistency_detector=self.inconsistency_detector
        )
    ...
    def get_unified_forecast(
        ...
        """
        logger.info(
            f"Obteniendo pronóstico fusionado para ({latitude}, {longitude}), {hours}h"
        )
        ...
        all_data = self._fetch_all_sources(
            latitude, longitude, hours, available_sources
        )
    ...
    def get_current_unified(
        self, latitude: float, longitude: float, sources: Optional[list[str]] = None
        ...
        """
        logger.info(
            f"Obteniendo condiciones actuales fusionadas para ({latitude}, {longitude})"
        )
            ...
            for source in available_sources:
                ...
                if repo:
                    future = executor.submit(
                        self._fetch_current_safe, repo, latitude, longitude, source
                    )
    ...
    def _get_available_sources(self) -> list[str]:
        """Retorna fuentes disponibles: Windy-GFS siempre; WRF-SMN si WRF_SMN_ENABLED=true."""
        sources: list[str] = [
            "windy_ecmwf",
            "windy_gfs",
        ]
        if self.repository_factory.get_repository("wrf_smn") is not None:
            sources.append("wrf_smn")
        return sources
    ...
    def _fetch_all_sources(
        self, latitude: float, longitude: float, hours: int, sources: list[str]
    ...
    def _fetch_forecast_safe(
        self, repo, latitude: float, longitude: float, hours: int, source: str
    ) -> list[NormalizedWeatherData]:
        """
        ...
        try:
            logger.info(
                f"Intentando obtener datos de {source} para ({latitude}, {longitude}), {hours}h"
            )
    ...
    def _fetch_current_safe(
        self, repo, latitude: float, longitude: float, source: str
        ...
        try:
            ...
            return self.normalizer.normalize_weather_data(
                data=raw_data, forecast_hour=0, source_override=source
            )
        except Exception as e:
    ...
    def _group_by_forecast_hour(
        self, data: list[NormalizedWeatherData]
    ...
    def get_model_comparison(
        ...
        for source in sources:
            repo = self._get_repository(source)
            if repo:
                data = self._fetch_forecast_safe(
