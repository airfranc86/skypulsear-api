...
class WeatherNormalizerService:
    ...
    def normalize_batch(
        ...
        for i, data in enumerate(data_list):
            ...
            normalized.append(
                self.normalize_weather_data(data, forecast_hour, source)
            )
    ...
    def _extract_temperature(self, data: dict) -> Optional[float]:
        ...
        temp = data.get("temperature") or data.get("temp") or data.get("temperature_celsius")
    ...
    def _extract_wind_direction(self, data: dict) -> Optional[float]:
        ...
        direction = data.get("wind_direction") or data.get("wind_dir") or data.get("wind_direction_deg")
        return self._to_float(direction)
    
    def _extract_precipitation(self, data: dict) -> Optional[float]:
        """Extrae precipitación en mm."""
        precip = data.get("precipitation") or data.get("precip") or data.get("precipitation_mm") or data.get("rain")
    ...
    def _extract_cloud_cover(self, data: dict) -> Optional[float]:
        ...
        clouds = data.get("cloud_cover") or data.get("clouds") or data.get("cloud_cover_pct") or data.get("cloudiness")
        return self._to_float(clouds)
    
    def _extract_humidity(self, data: dict) -> Optional[float]:
        """Extrae humedad en porcentaje."""
        humidity = data.get("humidity") or data.get("humidity_pct") or data.get("relative_humidity")
        return self._to_float(humidity)
    
    def _extract_pressure(self, data: dict) -> Optional[float]:
        """Extrae presión en hPa."""
        pressure = data.get("pressure") or data.get("pressure_hpa") or data.get("sea_level_pressure")
