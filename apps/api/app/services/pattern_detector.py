...
class PatternDetector:
    ...
    def detect_from_current(
        self, current: UnifiedForecast, cape: Optional[float] = None
    ) -> list[DetectedPattern]:
        """
        ...
        if (
            ...
            and current.temperature_celsius >= self.THRESHOLDS["extreme_heat"]
        ):
            patterns.append(self._create_extreme_heat_pattern(current))

        # Helada
        if (
            current.temperature_celsius is not None
            and current.temperature_celsius <= self.THRESHOLDS["frost"]
        ):
    ...
    def _detect_convective_storm(
        self, forecasts: list[UnifiedForecast], cape_values: Optional[list[float]]
    ...
    def _detect_storm_by_proxy(
        self, forecasts: list[UnifiedForecast]
    ) -> Optional[DetectedPattern]:
    ...
    def _detect_heat_wave(
        ...
        return DetectedPattern(
            ...
            thresholds_exceeded=[
                f"Temperatura > {self.THRESHOLDS['heat_wave_day']}°C por {high_temp_days:.1f} días"
            ],
    ...
    def _detect_cold_wave(
        ...
        return DetectedPattern(
            ...
            thresholds_exceeded=[
                f"Temperatura < {self.THRESHOLDS['cold_wave']}°C por {low_temp_days:.1f} días"
            ],
    ...
    def _detect_frost(
        ...
        return DetectedPattern(
            ...
            trigger_values={"min_temperature": min_temp, "frost_hours": frost_hours},
    ...
    def _create_storm_pattern(
        self, cape: float, current: UnifiedForecast
    ) -> Optional[DetectedPattern]:
    ...
    def _create_extreme_heat_pattern(self, current: UnifiedForecast) -> DetectedPattern:
    ...
    def _create_frost_pattern_single(self, current: UnifiedForecast) -> DetectedPattern:
