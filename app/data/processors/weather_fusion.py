...
class WeatherFusionProcessor:
    ...
    def fuse(
        ...
        """
        if not data_points:
            return self._create_empty_forecast(timestamp, forecast_hour, latitude, longitude)
    ...
    def _fuse_variable(
        ...
        adjusted_weights = self.inconsistency_detector.adjust_weights_for_inconsistencies(
        ...
        for source, value in source_values.items():
            weight = adjusted_weights.get(source, 0.1)  # Default bajo para fuentes desconocidas
        ...
        if var_inconsistencies:
            avg_severity = sum(i.severity for i in var_inconsistencies) / len(var_inconsistencies)
    ...
    def _source_to_weight_key(self, source: WeatherSource) -> str:
        ...
        mapping = {
            WeatherSource.METEOSOURCE: "meteosource",
            WeatherSource.METEOSOURCE_WRF: "wrf_smn",  # WRF de Meteosource usa pesos de WRF
            ...
            WeatherSource.LOCAL_STATIONS: "local_stations",
        }
        return mapping.get(source, "meteosource")
    ...
    def _weight_key_to_source(self, key: str) -> WeatherSource:
        ...
        return mapping.get(key, WeatherSource.METEOSOURCE)
